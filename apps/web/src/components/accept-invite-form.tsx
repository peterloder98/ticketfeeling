...
export function AcceptInviteForm({
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/v1/box-office/invites/accept", {
        method: "POST",
  ...
  return (
    <form onSubmit={onSubmit} className="tf-card mx-auto max-w-md space-y-4">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-semibold text-[var(--tf-navy)]">Zugang einrichten</h1>
      ...
      <button type="submit" className="tf-btn tf-btn-primary w-full" disabled={busy}>
        {busy ? "Richtet ein…" : "Zugang anlegen & zur Kasse"}
      </button>
    </form>
  );
}
